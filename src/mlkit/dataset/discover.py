...
def assign_splits(groups: list[str], val_ratio: float,
    ...
    Группы упорядочиваются по хэшу имени (одинаковый вход — одинаковый
    порядок между запусками и машинами), а доли отрезаются квотами. Чистое
    ...
    """
    ordered = sorted(set(groups), key=lambda key: hashlib.sha1(key.encode("utf-8")).hexdigest())
    total = len(ordered)
    if total == 0:
        return {}
